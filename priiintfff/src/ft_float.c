...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/30 20:32:06 by ythomas           #+#    #+#             */
/*   Updated: 2019/01/30 20:32:08 by ythomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_get_len_float(t_param *list, int i)
{
	...
	if (list->precision == -1)
	{
		len = i + 6 + list->flag->plus + list->flag->space;
		if (len < list->range)
			len = list->range;
	}
	else if (i + list->precision + list->flag->plus
		+ list->flag->space < list->range)
		len = list->range;
	else
		len = i + list->precision + list->flag->plus + list->flag->space;
	return (len);
}
...
char	*fill_buf(char *left, char *right, t_param *list, int len)
{
	...
	if (list->flag->minus == 1)
		i = ft_strlen(left) + ft_strlen(right);
	j = ft_strlen(right) - 1;
	while (j >= 0)
	{
		buf[i] = right[j];
